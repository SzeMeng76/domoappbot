...
from types import SimpleNamespace # ✨ 新增：这是让 App ID 查询能复用代码所必需的
...
async def handle_app_search_callback(
    ...
    # This is the original function from the user, unchanged.
    ...
    if not session or session.get("user_id") != user_id:
        ...
        return
    logger.info(f"🔍 Processing callback for session {session_id} (user={user_id}, chat={chat_id}): {query.data}")
    try:
        if action == "select":
            ...
            region_buttons = [
                InlineKeyboardButton("🇨🇳 中国", callback_data=f"app_region_CN_{session_id}"),
                InlineKeyboardButton("🇭🇰 香港", callback_data=f"app_region_HK_{session_id}"),
                InlineKeyboardButton("🇹🇼 台湾", callback_data=f"app_region_TW_{session_id}"),
                InlineKeyboardButton("🇯🇵 日本", callback_data=f"app_region_JP_{session_id}"),
                InlineKeyboardButton("🇬🇧 英国", callback_data=f"app_region_GB_{session_id}"),
                InlineKeyboardButton("❌ 关闭", callback_data=f"app_close_{session_id}"),
            ]
            ...
            page_results = all_results[0:per_page]
            search_data_for_session = {
                "query": final_query, "country": country_code.lower(), "app_type": app_type,
                "all_results": all_results, "current_page": 1, "total_pages": total_pages,
                "total_results": total_results, "per_page": per_page, "results": page_results,
            }
            ...
            keyboard = create_search_keyboard(search_data_for_session, session_id)
            await query.edit_message_text(
                foldable_text_v2(result_text), reply_markup=keyboard,
                parse_mode="MarkdownV2", disable_web_page_preview=True
            )
        elif (action == "back" and len(parts) > 2 and parts[2] == "to" and parts[3] == "search"):
            search_data = session["search_data"]
            result_text = format_search_results(search_data)
            keyboard = create_search_keyboard(search_data, session_id)
            await query.edit_message_text(foldable_text_v2(result_text), reply_markup=keyboard, parse_mode="MarkdownV2")
            ...
            logger.info(f"✅ 已取消会话 {session_id} 的 {cancelled_count} 个定时删除任务")
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=session["bot_response_message_id"])
                await context.bot.delete_message(chat_id=chat_id, message_id=session["user_command_message_id"])
                logger.info(f"✅ 已删除会话 {session_id} 的所有消息")
            except Exception as e:
                logger.error(f"❌ 删除消息时发生错误: {e}")
                await query.edit_message_text("🔍 搜索已关闭。\n\n使用 `/app 应用名称` 开始新的搜索。")
            if session_id in user_search_sessions:
                del user_search_sessions[session_id]
...
async def get_app_prices(
    ...
    if cached_data:
        cache_timestamp = cache_manager.get_cache_timestamp(cache_key, subdirectory="app_store")
        cache_info = (f"*(缓存于: {datetime.fromtimestamp(cache_timestamp).strftime('%Y-%m-%d %H:%M')})*" if cache_timestamp else "")
        return {
            "country_code": country_code,
            "country_name": SUPPORTED_COUNTRIES.get(country_code, {}).get("name", country_code),
            "flag_emoji": get_country_flag(country_code),
            "status": "ok", "app_price_str": cached_data.get("app_price_str"),
    ...
    try:
        ...
        if e.response.status_code == 404:
            ...
            return {"country_code": country_code, "country_name": country_name, "flag_emoji": flag_emoji, "status": "not_listed", "error_message": "未上架"}
        else:
            logger.error(f"HTTP error fetching prices for {app_name} in {country_code}: {e}")
            return {"country_code": country_code, "country_name": country_name, "flag_emoji": flag_emoji, "status": "error", "error_message": f"获取失败 (HTTP {e.response.status_code})"}
    except httpx.RequestError as e:
        logger.error(f"Failed to fetch prices for {app_name} in {country_code}: {e}")
        return {"country_code": country_code, "country_name": country_name, "flag_emoji": flag_emoji, "status": "error", "error_message": "获取失败 (网络错误)"}
    except Exception as e:
        logger.error(f"Unknown error fetching prices for {app_name} in {country_code}: {e}")
        return {"country_code": country_code, "country_name": country_name, "flag_emoji": flag_emoji, "status": "error", "error_message": "获取失败 (未知错误)"}
    try:
        try:
        ...
        result_data = {
            "country_code": country_code, "country_name": country_name, "flag_emoji": flag_emoji,
            "status": "ok", "app_price_str": app_price_str, "app_price_cny": app_price_cny, "in_app_purchases": in_app_purchases,
        }
        ...
        return {"country_code": country_code, "country_name": country_name, "flag_emoji": flag_emoji, "status": "error", "error_message": "解析失败"}
