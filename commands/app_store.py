...
from utils.message_manager import schedule_message_deletion, cancel_session_deletions
    ...
    async def search_apps(query: str, country: str = "us", app_type: str = "software", limit: int = 50) -> Dict:
        ...
        try:
            async with httpx.AsyncClient(verify=False) as client:
                ...
                if not data.get("results") and app_type != "software": # Only try fallback if not already general software
                    fallback_params = {
                        "term": query,
                        "country": country,
                        "media": "software",
...
async def app_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if user_id in user_search_sessions:
            ...
            if old_session_id:
                cancelled_count = cancel_session_deletions(old_session_id)
                logger.info(f"🔄 用户 {user_id} 有现有搜索会话，已取消 {cancelled_count} 个旧的删除任务")
            logger.info(f"🔄 User {user_id} has existing search session (message: {old_session.get('message_id')}, query: '{old_session.get('query')}'), will be replaced with new search")
        ...
        await message.edit_text(
            foldable_text_v2(search_status_message),
            parse_mode="MarkdownV2"
        )
        ...
        logger.info(f"✅ Created new search session for user {user_id}: message {message.message_id}, query '{final_query}', chat {update.effective_chat.id}, session {session_id}")
        ...
        logger.info(f"🔧 调度消息删除: 消息 {message.message_id} 将在 {delete_delay} 秒后删除")
        ...
        if task_id:
            logger.info(f"✅ 成功调度删除任务 {task_id} 用于搜索结果消息 {message.message_id}")
        else:
...
async def handle_app_search_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if data.startswith("app_select_"):
            ...
            raw_search_data = await SappSearchAPI.search_apps(final_query, country=country_code.lower(), app_type=app_type, limit=200)
            all_results = raw_search_data.get("results", [])
...
async def show_app_details(query, app_id: str, app_info: Dict, context: ContextTypes.DEFAULT_TYPE, session: Dict) -> None:
    try:
...
async def get_app_prices(app_name: str, country_code: str, app_id: int, app_name_from_store: str, app_type: str, context: ContextTypes.DEFAULT_TYPE) -> Dict:
    ...
    cached_data = cache_manager.load_cache(cache_key, max_age_seconds=config_manager.config.app_store_cache_duration, subdirectory="app_store")
    if cached_data:
...
# Register commands
command_factory.register_command("app", app_command, permission=Permission.USER, description="App Store应用搜索（支持iOS/macOS/iPadOS平台筛选）")
