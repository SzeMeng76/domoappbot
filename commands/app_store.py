...
async def get_app_prices(
    ...
    if cached_data:
        # Simplified return for brevity, original logic is preserved in the full code block
        return {**cached_data, "status": "ok", "country_code": country_code, "country_name": SUPPORTED_COUNTRIES.get(country_code, {}).get("name", country_code)}
...
def find_common_plan(all_price_data: list[Dict]) -> str | None:
    ...
    for price_data in all_price_data:
        if price_data["status"] == "ok":
            for iap in price_data.get("in_app_purchases", []):
                plan_counts[iap["name"]] = plan_counts.get(iap["name"], 0) + 1
    if not plan_counts: return None
    ...
    for keyword in ["Pro", "Premium", "Plus", "Standard"]:
        for plan in common_plans:
            if keyword in plan: return plan
...
def sort_key_func(price_data: Dict, target_plan: str | None = None) -> tuple[float, float]:
    if price_data["status"] != "ok": return (float("inf"), float("inf"))
    app_price = price_data.get("app_price_cny", float("inf"))
    target_plan_price, min_in_app_price = float("inf"), float("inf")
    for iap in price_data.get("in_app_purchases", []):
        cny_price = iap.get("cny_price")
        if cny_price is not None:
            if iap["name"] == target_plan: target_plan_price = cny_price
            min_in_app_price = min(min_in_app_price, cny_price)
    if target_plan_price != float("inf"): effective_price = target_plan_price
    elif min_in_app_price != float("inf"): effective_price = min_in_app_price
    else: effective_price = app_price
...
async def app_store_clean_cache_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        sent_message = await context.bot.send_message(chat_id=update.effective_chat.id, text=f"❌ 缓存清理失败: {str(e)}")
        schedule_message_deletion(chat_id=sent_message.chat_id, message_id=sent_message.message_id, delay=5)
